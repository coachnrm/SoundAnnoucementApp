...
window.generatePdfWithFont = (title, content, personsJson) => {
    ...
    fetch('/fonts/NotoSans-Regular.ttf')
        ...
        .then(fontData => {
            ...
            // Render the chart on a hidden canvas
            const canvas = document.createElement('canvas');
            canvas.width = 400;
            canvas.height = 200;
            ...
            setTimeout(() => {
                ...
                doc.addImage(chartImage, 'PNG', 10, yPosition + 10, 180, 90);
                document.body.removeChild(canvas);

                // Set the PDF to auto-print
                doc.autoPrint();
                ...
                iframe.onload = () => {
                    iframe.contentWindow.print(); // Auto-print the content of the iframe
                    document.body.removeChild(iframe); // Cleanup after printing
                };

                // Add the PDF to the iframe
                iframe.src = doc.output('bloburl');
            }, 1000); // Adjust if the chart takes longer to render
        })
        .catch(error => console.error('Error loading font:', error));
};
